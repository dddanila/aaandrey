...
async def __start(_: Update, chat_manager: ChatManager):
  ...
  await chat_manager.bot.send_message(chat_id=chat_id, text="Для начала отправь мне сообщение!")
...
async def __mode_add_cancel(_: Update, chat_manager: ChatManager) -> int:
  await chat_manager.bot.send_message(chat_id=chat_manager.context.chat_id, text="Создание режима отменено.")
...
async def __post_init(app: Application):
  commands = [
    ...
    ('history', "Показывать предыдущие разговоры"),
    ('retry', "Пересоздать запрос для последнего сообщения"),
    ('mode', "Выберите режим для текущего разговора"),
  ]
...
def run(token: str, gpt: GPTClient, chat_ids: list[int], conversation_timeout: int|None, data_path: str|None, webhook_info: WebhookInfo|None):
  ...
  with open('whitelist.txt', 'r') as file:
    allowed_chat_ids = [int(line.strip()) for line in file]
